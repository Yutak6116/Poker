...
def det_winner2(p1_hand,p2_hand,board):
    ...
    if det_winner(p1_cards,p2_cards) == 1:
        window.withdraw()
        messagebox.showinfo("result","DRAW!", parent = root)
        for i in range(2):
            tips[i] += pot / 2
            pot = 0
    if det_winner(p1_cards,p2_cards) == 0:
        window.withdraw()
        messagebox.showinfo("result","CPU WINS POT!", parent = root)
        winner = 1 
        tips[winner] += pot
        pot = 0
    ...
    while running:
        ...
        if gamescene == 0:
            ...
            if tips.count(0) != 1: # if_gamefinish == False
                ...
                messagebox.showinfo("Game Finished", "Pless OK")
                gamescene = 0
        else:
            print("error")    
        pygame.display.flip()        
