...
Original file is located at
    https://colab.research.google.com/drive/1vqRAD4f1tc8Nhqo15dcTb8_w27O0qOvF
"""
...
def if_gamefinish():
    global winner
    if tips.count(0) == how_many_p - 1:
        winner = tips.index(how_many_p * buy_in) 
        messagebox.showinfo("Game Finished", "Pless Enter Key")
        return True
    else:
        return False
...
def input_number():
    global money
    money = int(num_area.get())
    root.quit()
    root.destroy()
root = tkinter.Tk()
root.title("input")
root.geometry('400x200')
label = tkinter.Label(text='Enter how much you want to bet in total')
label.place(x=30, y=40)
num_area = tkinter.Entry(master=root, width=50)
num_area.place(x=30, y=70)
num_area.pack()
enter_btn = tkinter.Button(master=root, text='Enter', command=input_number)
enter_btn.place(x=30, y=100)
...
def dealer(a, now_board, screen):
    global tips, f_tips
    if if_noplay[a] or not tips[a]:
        return dealer((a + 1)% how_many_p, now_board, screen)
    else:
        if a == 0:
            player_action(a)
        else:
        ...
        if if_dealfinish() == 2:
            ...
            if if_next():
                ...
                dealer((a + 1)% how_many_p, now_board, screen)
    pygame.display.flip()
...
def tips_update(screen):
    ...
    text_f_tips1 = font2.render(str(f_tips[0]), True, (0,0,0))
    text_f_tips2 = font2.render(str(f_tips[1]), True, (0,0,0))
    text_tips1 = font2.render(str(tips[0]), True, (0,0,0))
    text_tips2 = font2.render(str(tips[1]), True, (0,0,0))
    text_pot = font2.render(str(pot), True, (0,0,0))
    ...
    button4 = pygame.Rect(320, 700, 70, 50)
    button5 = pygame.Rect(400, 700, 70, 50) 
    text1 = font1.render("CHECK", True, (0,0,0))
    text2 = font1.render("FOLD", True, (0,0,0))
    text3 = font1.render("CALL", True, (0,0,0))
    text4 = font1.render("BET", True, (0,0,0))
    text5 = font1.render("RAISE", True, (0,0,0))  
...
def main():
    ...
    if whoBB == 0:
        player1_position = "BB"
    elif whoBB == (0+1) % how_many_p:
        player1_position = "SB"
    else:
        player1_position = ""
    if whoBB == 1:
        player2_position = "BB"
    elif whoBB == (1+1) % how_many_p:
        player2_position = "SB"
    else:
        player2_position = ""
    text_position1 = font2.render(player1_position, True, (0,0,0)) 
    text_position2 = font2.render(player2_position, True, (0,0,0))

    playernamerect1 = Rect(480, 730, 80, 20)
    playernamerect2 = Rect(610, 60, 80, 20)
    text_playername1 = font2.render("player", True, (0,0,0))
    text_playername2 = font2.render("CPU", True, (0,0,0))
            ...
            if if_gamefinish() == False:
                ...
                if if_dealfinish() == 2:
                    ...
                    while if_dealfinish() == 0:
                        if len(now_board) == 0:
                            who_play = (whoBB + 1) % how_many_p
                            dealer(who_play, now_board, screen)
                        else:
                            who_play = (whoBB - 1) % how_many_p
                            dealer(who_play, now_board, screen)
            elif if_gamefinish() == True:
